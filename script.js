...
function resetAnimations(slide) {
    const animatedElements = slide.querySelectorAll('.list-item, .bucket, .role-item, .timeline-item, .stat-card, .principle');
...
function copyPromptText(button) {
    const promptCode = button.previousElementSibling;
    const promptText = promptCode.innerText.trim().replace(/^"|"$/g, '');
...
function copyMasterPrompt(button) {
    const promptCode = button.previousElementSibling;
    const promptText = promptCode.innerText.trim().replace(/^"|"$/g, '');
    ...
    navigator.clipboard.writeText(promptText).then(() => {
        ...
        button.innerText = 'Failed';
        setTimeout(() => {
            button.innerText = 'Copy Master Prompt';
        }, 2000);
    });
}
